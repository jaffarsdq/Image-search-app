...
function App() {
  ...
  return (
    <div className="container py-5">
      <h1 className="text-center m-0 title">Image Search App</h1>
        ...
        <div className="pb-5">
          <div className="input-wrapper">
            <input
              ...
              value={query}
            />
            <button onClick={() => setSearchQ(query)} className="search">
          ...
          {searchQ && <div onClick={() => reset()} className="reset-btn">{('Reset')}</div>}
      ...
      {searchQ ?
        ...
        <Image url={img.urls.thumb} key={key} />
      )) : data.map((img, key) => (
        <Image url={img.urls.thumb} key={key} />
      ))}
      </div>
